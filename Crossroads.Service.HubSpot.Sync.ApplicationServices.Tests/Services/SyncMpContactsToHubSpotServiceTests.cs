...
namespace Crossroads.Service.HubSpot.Sync.ApplicationServices.Test.Services
{
    public class SyncMpContactsToHubSpotServiceTests
    {
        ...
        private void SetUpDefaults()
        {
            ...
            _hubSpotSyncerMock.Setup(syncer => syncer.BulkSync(It.IsAny<BulkHubSpotContact[]>(), 1000)).Returns(Task.FromResult(new BulkSyncResult()));
            _hubSpotSyncerMock.Setup(syncer => syncer.BulkSync(It.IsAny<BulkHubSpotContact[]>(), 100)).Returns(Task.FromResult(new BulkSyncResult()));
            _hubSpotSyncerMock.Setup(syncer => syncer.BulkSync(It.IsAny<BulkHubSpotContact[]>(), 10)).Returns(Task.FromResult(new BulkSyncResult()));
            _hubSpotSyncerMock.Setup(syncer => syncer.SerialCreateAsync(It.IsAny<SerialHubSpotContact[]>())).Returns(Task.FromResult(new SerialSyncResult()));
            _hubSpotSyncerMock.Setup(syncer => syncer.SerialUpdateAsync(It.IsAny<SerialHubSpotContact[]>())).Returns(Task.FromResult(new SerialSyncResult()));
            _hubSpotSyncerMock.Setup(syncer => syncer.ReconcileConflicts(It.IsAny<SerialHubSpotContact[]>())).Returns(Task.FromResult(new SerialSyncResult()));
            _clockMock.Setup(clock => clock.UtcNow).Returns(utcNowMockDateTime);
            _configSvcMock.Setup(svc => svc.GetCurrentActivityProgressAsync()).Returns(Task.FromResult(new ActivityProgress { ActivityState = ActivityState.Idle}));
            _configSvcMock.Setup(svc => svc.GetLastSuccessfulOperationDatesAsync()).Returns(Task.FromResult(operationDates));
            _configSvcMock.Setup(svc => svc.PersistActivity()).Returns(true);
            _jobRepoMock.Setup(repo => repo.PersistActivityProgressAsync(It.IsAny<ActivityProgress>())).Returns(Task.CompletedTask);
            _jobRepoMock.Setup(repo => repo.PersistLastSuccessfulOperationDatesAsync(It.IsAny<OperationDates>())).Returns<OperationDates>(Task.FromResult);
            _jobRepoMock.Setup(repo => repo.PersistHubSpotApiDailyRequestCountAsync(It.IsAny<int>(), It.IsAny<DateTime>())).Returns(Task.CompletedTask);
            _jobRepoMock.Setup(repo => repo.PersistActivityAsync(It.IsAny<Activity>())).Returns(Task.CompletedTask);
        ...
        public async Task HappyPath()
        {
            ...
            _hubSpotSyncerMock.Verify(syncer => syncer.SerialCreateAsync(It.IsAny<SerialHubSpotContact[]>()), Times.Exactly(3));
            _hubSpotSyncerMock.Verify(syncer => syncer.SerialUpdateAsync(It.IsAny<SerialHubSpotContact[]>()), Times.Exactly(3));
            _clockMock.Verify(clock => clock.UtcNow, Times.Exactly(10));
            _configSvcMock.Verify(svc => svc.GetCurrentActivityProgressAsync(), Times.Once);
            _configSvcMock.Verify(svc => svc.GetLastSuccessfulOperationDatesAsync(), Times.Once);
            _jobRepoMock.Verify(repo => repo.PersistActivityProgressAsync(It.IsAny<ActivityProgress>()), Times.Exactly(14));
            _jobRepoMock.Verify(repo => repo.PersistLastSuccessfulOperationDatesAsync(It.IsAny<OperationDates>()), Times.Exactly(4));
            _jobRepoMock.Verify(repo => repo.PersistHubSpotApiDailyRequestCountAsync(It.IsAny<int>(), It.IsAny<DateTime>()), Times.Exactly(3));
            _jobRepoMock.Verify(repo => repo.PersistActivityAsync(It.IsAny<Activity>()), Times.Once);
